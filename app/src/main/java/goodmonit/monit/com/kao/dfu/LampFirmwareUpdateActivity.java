...
public class LampFirmwareUpdateActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mLampDevice.getConnectionState() == DeviceConnectionState.BLE_CONNECTED && mLampDevice.getServerDirectConnectionState() == false) {
            if (mDlgDfuError == null) {
                mDlgDfuError = new SimpleDialog(LampFirmwareUpdateActivity.this,
    ...
    private void checkUpdateAvailable() {
        ...
        tvCurrentVersion.setText(getString(R.string.current_version) + " : " + mLampDevice.getDeviceInfo().firmwareVersion);
        tvLatestVersion.setText(getString(R.string.latest_version) + " : " + mPreferenceMgr.getLampVersion());
        if (versionMgr.checkUpdateAvailable(mLampDevice.getDeviceInfo().firmwareVersion, mPreferenceMgr.getLampVersion()) || Configuration.MASTER) {
    ...
    protected void onResume() {
        ...
        if (isOTAUpdating) {
            ...
            if ((mLampDevice.getConnectionState() == DeviceConnectionState.DISCONNECTED)) {
                finish();
            }
        }
    }
                ...
                new ServerManager.ServerResponseListener() {
                    ...
                    public void onReceive(int responseCode, String errCode, String data) {
                        if (InternetErrorCode.SUCCEEDED.equals(errCode)) {
                            ...
                            prevFirmwareVersion = mLampDevice.firmwareVersion;
        ...
        public void handleMessage(Message msg) {
            switch(msg.what) {
                ...
                case MSG_OTA_UPDATE_PROGRESS:
                    ...
                    if (DBG) Log.d(TAG, "MSG_OTA_UPDATE_PROGRESS : " + mOtaUpdateProgress + "/300 -> " + (mOtaUpdateProgress / 3) + "% / " + prevFirmwareVersion + " / " + mLampDevice.firmwareVersion);
                    mOtaUpdateProgress++;
                    if (mOtaUpdateProgress < 10) {
                        ...
                        if (prevFirmwareVersion != null && !prevFirmwareVersion.equals(mLampDevice.firmwareVersion)) {
                            if (DBG) Log.d(TAG, "Update Succeeded : " + prevFirmwareVersion + " -> " + mLampDevice.firmwareVersion);
                ...
                case ConnectionManager.MSG_WIFI_CONNECTION_STATE_CHANGE:
                    ...
                    if (deviceInfo.type == DeviceType.LAMP && mConnectedDeviceId == deviceInfo.deviceId) {
