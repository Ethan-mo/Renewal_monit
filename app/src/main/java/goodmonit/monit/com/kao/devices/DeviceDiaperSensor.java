...
public class DeviceDiaperSensor extends DeviceInfo {
    private static final String TAG = Configuration.BASE_TAG + "DiaperSensor";
    ...
    public DeviceDiaperSensor(Context context) {
        super();
        mContext = context;
        mPreferenceMgr = PreferenceManager.getInstance(context);
        type = DeviceType.DIAPER_SENSOR;
        switch (Configuration.APP_MODE) {
            ...
            case Configuration.APP_MONIT_X_KAO:
                name = DIAPER_SENSOR_BASE_NAME;
                break;
            case Configuration.APP_KC_HUGGIES_X_MONIT:
                name = KC_DIAPER_SENSOR_BASE_NAME;
                break;
        }
    ...
    public DeviceDiaperSensor(Context context, DeviceInfo info) {
...
//            }
    ...
    public void setSleepStatus(int status) {
        if(DBG) Log.d(TAG, "sleepStatus : " + status);
        if (status != -1) {
            sleepStatus = status;
        }
    }
