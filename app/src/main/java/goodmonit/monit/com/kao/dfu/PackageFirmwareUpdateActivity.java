...
public class PackageFirmwareUpdateActivity extends BaseActivity {
    private static final String TAG = Configuration.BASE_TAG + "PkgDFU";
    private static final boolean DBG = Configuration.DBG;

    private static final int REQCODE_BLE_DIRECT_CONNECTION  = 1;

    private static final int MSG_SENSOR_OTA_UPDATE          = 1;
    private static final int MSG_HUB_OTA_UPDATE_PROGRESS    = 2;
    private static final int MSG_HUB_OTA_UPDATE_COMPLETED   = 3;
    ...
    public static final int HUB_UPDATE_TIME_SEC             = 300;
    ...
    private String mPrevHubFirmwareVersion;
    ...
    private Button btnUpdateLowerVersion, btnUpdateHigherVersion;
    ...
    private boolean mUpdateSensor, mUpdateSensorForce;
    private boolean mUpdateHub, mUpdateHubForce;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = this;
        mConnectedSensorDeviceId = getIntent().getLongExtra("targetSensorDeviceId", -1);
        mConnectedHubDeviceId = getIntent().getLongExtra("targetHubDeviceId", -1);
        ...
        mHubDevice = ConnectionManager.getDeviceAQMHub(mConnectedHubDeviceId);
        mOTAMode = 0;

        if (mMonitSensor == null || mHubDevice == null) {
            if (DBG) Log.e(TAG, "targetDevice NULL : " + mConnectedSensorDeviceId + " / " + mConnectedHubDeviceId);
            finish();
            return;
        }
        ...
        if (mMonitSensor.getConnectionState() != DeviceConnectionState.BLE_CONNECTED) {
            Intent intent = new Intent(PackageFirmwareUpdateActivity.this, GuideDirectConnectionActivity.class);
    ...
    private void checkUpdateAvailable() {
        ...
        if (!mUpdateSensorForce && !mUpdateSensor && !mUpdateHubForce && !mUpdateHub) {
            if (DBG) Log.e(TAG, "No Update Available");
            ...
            if (mUpdateSensorForce || mUpdateHubForce) { // 1. 강제업데이트시
                tvUpdateDescription.setText(getString(R.string.dfu_update_available_description_force));
                if (mUpdateSensorForce && mUpdateHubForce) { // 둘다인 경우
                    tvCurrentVersion.setText("");
                    tvLatestVersion.setText("");
                } else if (mUpdateSensorForce) {
                    tvCurrentVersion.setText("");
                    tvLatestVersion.setText("");
                } else if (mUpdateHubForce) {
                    tvCurrentVersion.setText("");
                    tvLatestVersion.setText("");
                } else {
                    tvCurrentVersion.setText("");
            ...
            } else if (mUpdateSensor && mUpdateHub) { // 2. 둘다 업데이트시
                tvUpdateDescription.setText(getString(R.string.dfu_update_available_description));
                tvCurrentVersion.setText(getString(R.string.device_type_diaper_sensor) + ": " + mMonitSensor.getDeviceInfo().firmwareVersion + " / " + mPreferenceMgr.getDiaperSensorVersion());
                tvLatestVersion.setText(getString(R.string.device_type_hub) + ": " + mHubDevice.getDeviceInfo().firmwareVersion + " / " + mPreferenceMgr.getHubVersion());
            } else if (mUpdateSensor) { // 3. 센서만 업데이트시
                tvUpdateDescription.setText(getString(R.string.dfu_update_available_description));
                tvCurrentVersion.setText(getString(R.string.device_type_diaper_sensor) + ": " + mMonitSensor.getDeviceInfo().firmwareVersion + " / " + mPreferenceMgr.getDiaperSensorVersion());
                tvLatestVersion.setText(getString(R.string.device_type_hub) + ": " + getString(R.string.latest_version));
            } else if (mUpdateHub) { // 4. 허브만 업데이트시
                tvUpdateDescription.setText(getString(R.string.dfu_update_available_description));
                tvCurrentVersion.setText(getString(R.string.device_type_diaper_sensor) + ": " + getString(R.string.latest_version));
                tvLatestVersion.setText(getString(R.string.device_type_hub) + ": " + mHubDevice.getDeviceInfo().firmwareVersion + " / " + mPreferenceMgr.getHubVersion());
            }
        }
    }
    ...
    protected void onResume() {
        ...
        mUpdateSensor = mVersionMgr.checkUpdateAvailable(mMonitSensor.getDeviceInfo().firmwareVersion, mPreferenceMgr.getDiaperSensorVersion());
        mUpdateSensorForce = mVersionMgr.checkUpdateAvailable(mMonitSensor.getDeviceInfo().firmwareVersion, mPreferenceMgr.getDiaperSensorForceVersion());

        mUpdateHub = mVersionMgr.checkUpdateAvailable(mHubDevice.getDeviceInfo().firmwareVersion, mPreferenceMgr.getHubVersion());
        mUpdateHubForce = mVersionMgr.checkUpdateAvailable(mHubDevice.getDeviceInfo().firmwareVersion, mPreferenceMgr.getHubForceVersion());
        ...
        btnUpdate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (btnUpdate.isSelected()) { // cancel
                    ...
                    if (mUpdateSensor || mUpdateSensorForce) { // 센서 업데이트시에는 센서 파워 체크
                        ...
                        if (responseCode == HttpURLConnection.HTTP_OK) {
                            if (InternetErrorCode.SUCCEEDED.equals(errCode)) {
                                if (mDfuCancelled) {
                                    ...
                                    mHandler.sendEmptyMessage(MSG_SENSOR_OTA_UPDATE);
                                }
                            } else {
    ...
    private void _doSensorOTAUpdate() {
        if (DBG) Log.d(TAG, "doSensorOTAUpdate: " + mUpdateSensor + " / " + mUpdateSensorForce);
        if (!mUpdateSensor && !mUpdateSensorForce) {
            if (DBG) Log.e(TAG, "no update available");
        } else {
    ...
    private void _doHubOTAUpdate() {
        if (DBG) Log.d(TAG, "doHubOTAUpdate: " + mUpdateHub + " / " + mUpdateHubForce);

        if (!mUpdateHub && !mUpdateHubForce) {
            _doSensorOTAUpdate();
        } else {
            isOTAUpdating = true;
            mServerQueryMgr.OTAUpdateDevice(
                    ...
                    new ServerManager.ServerResponseListener() {
                        ...
                        public void onReceive(int responseCode, String errCode, String data) {
                            if (InternetErrorCode.SUCCEEDED.equals(errCode)) {
                                if (DBG)
                                    Log.d(TAG, "Hub OTA Update Start: " + mHubDevice.firmwareVersion);
                                mOtaUpdateProgress = 0;
                                mPrevHubFirmwareVersion = mHubDevice.firmwareVersion;
                                mHandler.sendEmptyMessage(MSG_HUB_OTA_UPDATE_PROGRESS);
                            } else {
                                if (DBG) Log.d(TAG, "Hub OTA Update Failed");
                            }
                        }
                    });
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch(msg.what) {
                case MSG_HUB_OTA_UPDATE_COMPLETED:
                    removeMessages(MSG_HUB_OTA_UPDATE_COMPLETED);
                    mOtaUpdateProgress = mOtaUpdateProgress + 3;
                    if ((int)(mOtaUpdateProgress / 3) > 100) {
                        if (DBG) Log.d(TAG, "Hub Update Succeeded : " + mPrevHubFirmwareVersion + " -> " + mHubDevice.firmwareVersion);

                        // 센서 업데이트가 필요하면, 센서 업데이트 시작
                        if (mUpdateSensor || mUpdateSensorForce) {
                            _doSensorOTAUpdate();
                        } else {
                            ...
                            if (mDlgUpdateCompleted != null && !mDlgUpdateCompleted.isShowing()) {
                                try {
                                    mDlgUpdateCompleted.setContents(getString(R.string.dfu_status_completed));
                        ...
                        tvUploadStatus.setText("(" + (int)(mOtaUpdateProgress / 3) + "%)");
                        mHandler.sendEmptyMessageDelayed(MSG_HUB_OTA_UPDATE_COMPLETED, 100);
                    }
                    break;
                case MSG_SENSOR_OTA_UPDATE:
                    switch (mDfuStep) {
                ...
                case ConnectionManager.MSG_BLE_CONNECTION_STATE_CHANGE:
                    ...
                    break;
                case MSG_HUB_OTA_UPDATE_PROGRESS:
                    removeMessages(MSG_HUB_OTA_UPDATE_PROGRESS);
                    if (DBG) Log.d(TAG, "MSG_HUB_OTA_UPDATE_PROGRESS : " + mOtaUpdateProgress + " / " + HUB_UPDATE_TIME_SEC + " -> " + (mOtaUpdateProgress / 3) + "% / " + mPrevHubFirmwareVersion + " / " + mHubDevice.firmwareVersion);
                    mOtaUpdateProgress++;
                    if (mOtaUpdateProgress < 10) {
                        tvStatus.setText(getString(R.string.dfu_update_waiting));
                        tvUploadStatus.setText("");
                        mHandler.sendEmptyMessageDelayed(MSG_HUB_OTA_UPDATE_PROGRESS, 1000);
                    } else if (mOtaUpdateProgress < HUB_UPDATE_TIME_SEC) {
                        ...
                        if (mPrevHubFirmwareVersion != null && !mPrevHubFirmwareVersion.equals(mHubDevice.firmwareVersion)) {
                            if (DBG) Log.d(TAG, "Update Succeeded : " + mPrevHubFirmwareVersion + " -> " + mHubDevice.firmwareVersion);
                            mHandler.sendEmptyMessage(MSG_HUB_OTA_UPDATE_COMPLETED);
                        } else {
                            mHandler.sendEmptyMessageDelayed(MSG_HUB_OTA_UPDATE_PROGRESS, 1000);
                        }
                    } else {
                ...
                case ConnectionManager.MSG_WIFI_CONNECTION_STATE_CHANGE:
                    ...
                    if (DBG) Log.d(TAG, "MSG_WIFI_CONNECTION_STATE_CHANGE : [" + deviceInfo2.type + " / " + deviceInfo2.deviceId + "/" + mConnectedHubDeviceId + "] " + wifiConnectionState + " / " + deviceInfo2.firmwareVersion);
                    if (deviceInfo2.type == DeviceType.AIR_QUALITY_MONITORING_HUB && mConnectedHubDeviceId == deviceInfo2.deviceId) {
                        if (wifiConnectionState == DeviceConnectionState.WIFI_CONNECTED) {
                            ...
                            if (mPrevHubFirmwareVersion != null && !mPrevHubFirmwareVersion.equals(deviceInfo2.firmwareVersion)) {
                                mHandler.sendEmptyMessage(MSG_HUB_OTA_UPDATE_COMPLETED);
                            } else {
                                btnUpdate.setEnabled(true);
                                btnUpdate.setText(R.string.btn_update);
                                // 업데이트 실패
                                if (DBG) Log.e(TAG, "Update Failed : " + mPrevHubFirmwareVersion + " -> " + deviceInfo2.firmwareVersion);
                                if (mDlgUpdateFailed != null && !mDlgUpdateFailed.isShowing()) {
                                    try {
                                        mDlgUpdateFailed.show();
                                    } catch (Exception e) {
