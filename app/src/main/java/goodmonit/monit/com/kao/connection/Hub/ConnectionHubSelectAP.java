...
public class ConnectionHubSelectAP extends BaseFragment {
    private static final String TAG = Configuration.BASE_TAG + "SelectAP";
	...
	private void _initView(View v) {
		...
		if (mDlgFailedToGetScanList == null) {
			...
			mDlgFailedToGetScanList.setContentsGravity(Gravity.LEFT);
		}
		mDlgFailedToGetScanList.showHelpButton(true);
		mDlgFailedToGetScanList.setHelpButtonListener(new View.OnClickListener() {
			@Override
			...
			convertView.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (result.index == PARAM_FOR_NEW_NETWORK) {
						((ConnectionActivity) mMainActivity).showFragment(ConnectionActivity.STEP_HUB_ADD_NETWORK);
					} else {
						if (result.securityType == NetworkSecurityType.NONE) {
							...
							((ConnectionActivity) mMainActivity).showFragment(ConnectionActivity.STEP_HUB_INPUT_PASSWORD);
						}
					}
				}
			});
