...
public class EnvironmentGraphView extends View {
	 ...
	 */
	private Paint mGuideLinePaint, mTimePaint, mLineGraphPaint;
	...
	public EnvironmentGraphView(Context context) {
		super(context);
		mContext = context;
		init();
	}

    public EnvironmentGraphView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
		init();
    }
	...
	public void _drawInfo(Canvas canvas) {
		...
		if (tvCurrentTime != null) {
			tvCurrentTime.setText(DateTimeUtil.getStringSpecificTimeInDay(DateTimeUtil.TYPE_COLON, mBeginTimeMs + idx * 10 * 60 * 1000));
		}
	}
	...
	private void _drawLineGraph(Canvas canvas) {
		...
		for (Double curr : mValues) {
			...
			if (prev == -999) {
				path.moveTo(width, nowHeight);
			} else {
				path.moveTo(width, beforeHeight);
			}
			width += mWidthBetweenMinute;
			path.lineTo(width, nowHeight);
	...
	private void _drawVerticalGuideLine(Canvas canvas) {
		...
		mGuideLinePaint.setStrokeWidth((float)0.5);
		canvas.drawPath(path, mGuideLinePaint);
	}
